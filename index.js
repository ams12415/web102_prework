...
const unfunded_games_count = GAMES_JSON.filter((game) => {
    return game.pledged < game.goal;
}).length;

// create a string that explains the number of unfunded games using the ternary operator
const display_string = `A total of $${totalRaised.toLocaleString('en-US')} has been raised for ${GAMES_JSON.length} games. Currently, ${unfunded_games_count} ${unfunded_games_count === 1 ? "game remains" : "games remain"} unfunded. We need your help to fund these amazing games!`;

// create a new DOM element containing the template string and append it to the description container

const new_description = document.createElement("p");
new_description.innerHTML = display_string;
descriptionContainer.appendChild(new_description);
